...
This portfolio was built to demonstrate that breadth as a coherent body of work rather than as isolated scripts. It is organised as seven independent repositories connected by explicit dependencies, so that each can be read on its own while the whole forms a pipeline from first-principles simulation to decision-ready resource estimates. The remainder of this document summarises the arc, the engineering standards shared across the repositories, and the principal quantitative findings.
